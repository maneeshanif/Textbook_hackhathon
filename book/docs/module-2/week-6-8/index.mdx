...
sidebar_position: 1
description: "Mathematical foundations of robot motion — coordinate frames, transformations, kinematics, and dynamics"
module: 2
week: 6
...
1. **[2.1.1 — Forward Kinematics](/docs/module-2/week-6-8/forward-kinematics)** (3-4 hours)  
   Coordinate frames, DH parameters, and computing end-effector positions

2. **[2.1.2 — Inverse Kinematics](/docs/module-2/week-6-8/inverse-kinematics)** (3-4 hours)  
   Analytical and numerical solutions for joint angles from desired poses

3. **[2.1.3 — Robot Dynamics](/docs/module-2/week-6-8/dynamics)** (3-4 hours)  
   Forces, torques, and equations of motion for multi-body systems

4. **[2.1.4 — Balance and ZMP](/docs/module-2/week-6-8/balance-zmp)** (3-4 hours)  
...
```mermaid
graph LR
    A[Joint Angles θ₁...θₙ] -->|Forward Kinematics| B[End-Effector Position x,y,z]
...
```mermaid
graph LR
    A[Desired Trajectory] -->|Inverse Dynamics| B[Required Torques τ₁...τₙ]
...
Represent position + orientation in 4×4 matrix:
...
Where:
- $\dot{x}$ = linear velocity (3×1)
- $\omega$ = angular velocity (3×1)
- $J(q)$ = Jacobian matrix (6×N)
- $\dot{q}$ = joint velocities (N×1)
...
- ZMP on boundary → Marginally stable
- ZMP outside → Falling (must take recovery action)
