---
title: "3.1.3 — Actuator Technologies"
sidebar_label: "3.1.3 — Actuator Technologies"
sidebar_position: 3
...
# 3.1.3 — Actuator Technologies

> **Summary**: Explore actuator technologies that power humanoid robots — from electric motors to hydraulics and compliant actuators.
...
class MotorParameters:
    ...
    R: float = 1.0           # Resistance [Ω]
    L: float = 0.001         # Inductance [H]
    J: float = 0.0001        # Rotor inertia [kg⋅m²]
    B: float = 0.0001        # Viscous friction [Nm⋅s/rad]
...
class BLDCMotor:
    ...
    def step(self, load_torque: float = 0.0):
        ...
        # τ_motor = Kt*I              (Torque generation)
        # J*dω/dt = τ_motor - B*ω - τ_load  (Mechanical)
        
        # Back-EMF
        back_emf = self.params.Ke * self.velocity
        
        # Current dynamics: dI/dt = (V - I*R - Ke*ω) / L
        dI_dt = (self.voltage - self.current * self.params.R - back_emf) / self.params.L
        
        # Motor torque
        motor_torque = self.params.Kt * self.current
        
        # Velocity dynamics: dω/dt = (τ_motor - B*ω - τ_load) / J
    ...
    for t in np.linspace(0, 1, 100):
        ...
        if int(t * 100) % 10 == 0:
            ...
            print(f"t={t:.2f}s: pos={state['position']:.3f} rad, "
                  ...
                  f"τ={state['torque']:.2f} Nm")
```

---

## 🎯 Part 2: Series Elastic Actuators (SEA)
...
class SeriesElasticActuator:
    ...
    def step(self, external_load: float = 0.0):
        ...
        # Load dynamics: J*θ̈ = τ_spring - τ_external
...
if __name__ == '__main__':
    ...
    for t in np.linspace(0, 2, 200):
        ...
        if int(t * 100) % 20 == 0:
            state = sea.get_state()
            print(f"t={t:.2f}s: τ_out={state['output_torque']:.2f} Nm, "
...
class HydraulicActuator:
    ...
    def __init__(self, piston_area=0.001, max_pressure=20e6, dt=0.001):
        ...
        Args:
            piston_area: Piston cross-sectional area [m²]
...
### 🎯 Exercise: Actuator Selection and Design
...
**Success Criteria:**
- ✅ Meets torque requirements (100 Nm peak)
- ✅ Meets speed requirements (10 rad/s)
- ✅ Under weight limit (1.5 kg)
...
**[3.1.4 — Sensor Fusion →](./04-sensor-fusion)**
