...
export function GuestLimitBadge() {
  const { user } = useAuth();
  const { interactionCount, isLimitReached } = useGuest();
  ...
  const percentage = (interactionCount / GUEST_INTERACTION_LIMIT) * 100;
  ...
  return (
    <div
      ...
      `}
      aria-label={`Guest interactions: ${interactionCount} of ${GUEST_INTERACTION_LIMIT} used`}
    >
      <svg
      ...
      <span>
        {interactionCount}/{GUEST_INTERACTION_LIMIT} free questions
      </span>
    </div>
  );
}
