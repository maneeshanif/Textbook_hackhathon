---
title: "2.1.2 — Inverse Kinematics"
sidebar_label: "2.1.2 — Inverse Kinematics"
sidebar_position: 2
...
> **Summary**: Learn analytical and numerical methods to compute joint configurations for desired end-effector positions — the foundation of robot motion planning.
...
class TwoLinkArm:
    ...
    def inverse_kinematics(self, x, y, elbow_up=True):
        """Analytical IK: target position → joint angles.
...
if __name__ == '__main__':
    ...
    if result_up:
        ...
        print(f"Elbow-up: θ1={np.degrees(theta1_up):.1f}°, θ2={np.degrees(theta2_up):.1f}°")
    ...
    if result_down:
        ...
        print(f"Elbow-down: θ1={np.degrees(theta1_down):.1f}°, θ2={np.degrees(theta2_down):.1f}°")
        arm.plot_arm(theta1_down, theta2_down, target)
```

**Output:**
```
Elbow-up: θ1=21.8°, θ2=59.0°
FK verification error: 0.000000 m
Elbow-down: θ1=55.3°, θ2=-59.0°
```
...
if __name__ == '__main__':
    ...
    if solution is not None:
        print(f"\nSolution: θ1={np.degrees(solution[0]):.1f}°, θ2={np.degrees(solution[1]):.1f}°")
...
class HumanoidArm6DOF:
    ...
    def jacobian(self, theta):
        """Compute 3×6 Jacobian (numerical differentiation)."""
        epsilon = 1e-6
        J = np.zeros((3, 6))
...
✅ **Numerical IK**: Jacobian-based iteration for complex robots  
✅ **Damped Least Squares**: Handles singularities robustly  
✅ **Multi-Task IK**: Weighted combination of task objectives  
✅ **Constraints**: Joint limits, self-collision, balance

---

## 🔗 Next Section

Continue to [2.1.3 — Robot Dynamics](./03-dynamics) to learn equations of motion and dynamic simulation.

---

## 📖 References

1. Siciliano, B. et al. (2016). *Robotics: Modelling, Planning and Control* — Chapters 3-4
