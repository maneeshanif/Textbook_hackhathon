---
title: "2.2.3 — Terrain Adaptation and Footstep Planning"
sidebar_label: "2.2.3 — Terrain Adaptation"
sidebar_position: 3
...
# 2.2.3 — Terrain Adaptation and Footstep Planning
...
## 📖 Theory: Adaptive Locomotion

### The Terrain Challenge

**Flat Ground Assumptions Break:**
- Uneven terrain → variable foot heights
- Slopes → shifted center of mass
- Stairs → discrete height changes
- Obstacles → constrained foot placement
...
class TerrainMapper:
    ...
    def compute_terrain_normals(self) -> np.ndarray:
        ...
        Returns:
            (grid_h × grid_w × 3) array of normal vectors
        """
        # Smooth heightmap
        ...
        # Normal vectors: (-∂z/∂x, -∂z/∂y, 1) normalized
...
## 🎯 Part 2: Footstep Planning

### A* Search for Footsteps

**State Space:**
- State: $(x, y, \theta, \text{foot})$ where foot ∈ \{left, right\}
...
class FootstepPlanner:
    ...
    def plan(self, start: Tuple[float, float, float], 
        ...
        Args:
            start: (x, y, θ) initial stance
            goal: (x, y, θ) goal stance
            max_steps: Maximum number of steps
        
        Returns:
            List of footsteps: [(x, y, θ, foot), ...]
        """
...
if __name__ == '__main__':
    ...
    if footsteps:
        ...
        for i, (x, y, theta, foot) in enumerate(footsteps[:10]):
            ...
            print(f"  Step {i}: {foot:5s} at ({x:.2f}, {y:.2f}, {h:.2f}m) θ={theta:.1f}°")
    else:
        print("No path found!")
```
...
### 🎯 Exercise: Complete Terrain-Adaptive Walking System
...
**Success Criteria**:
- ✅ Robot climbs 5-step staircase without falling
- ✅ Footsteps follow planned trajectory
- ✅ Maintains balance throughout (no tipping)
- ✅ Adapts gait based on terrain perception
...
**[Module 3: Sensors & Actuators →](../../module-3/)**
